...
var ratio = {
    ...
    init: function() {
		ratio.texts = {
			...
			perVcMs: ratio.filterMan(1,jr.translate('Show per VC'), jr.translate('Show per VMS')),
			perTimePer: ratio.filterMan(2,jr.translate('Last 24h'), jr.translate('Last week'), jr.translate('Last 21 days'), jr.translate('Max days'))
		};
		ratio.perAll = [ratio.texts.selMsVc, ratio.texts.perVcMs, ratio.texts.selPeriod, ratio.texts.perTimePer];
		ratio.perAll[1].def=1;
		ratio.perAll[3].def=1;
		ratio.mapMajorCodes = [0,1,2,3,4,5,6,7,9,12,13,14,205];
		ratio.now=new Date();
    },
	alarmListener:
		{ addListener: { typeName: 'ratioData', method: function( data ) {
			new ratio.instance( this, data );
		} } },
		...
		var d=new JsSerilz('$',data), o, ar, key, max, dayInd=-1,
			...
			onToolTipClick = function(){
				if(!selectedMajorCode){
					var i=ratio.perAll[5].def=ratio.mapMajorCodes.indexOf(this.data),col=Math.floor(i/4),row=i%4+1;
					// Patch...
...
//				mouseBox.show(makeDiv("Hej"),'mouseBoxDiv');
			...
			onFilterChange = function(grp,val) {
				...
				while(++i<ratio.perAll[ind].lab.length&&ratio.perAll[ind].lab[i]!=val){}
				if(i<ratio.perAll[ind].lab.length)
					ratio.perAll[ind].def=i;
				if(isClick){
					tempIndex=ind;
					tempVal=i;
				}
			...
			onTempFilterIn = function() {
				...
				tempVal=ratio.perAll[tempIndex].def;
				onFilterChange(this.name,this.textContent);
			},
			onTempFilterOut = function() {
				ratio.perAll[tempIndex].def=tempVal;
				recalculate();
			},
			...
			getChecked = function(head,cont,isNotFirst) {
				...
				if(ncol==1){
					...
					if(!isNotFirst)
						rv.push({'tr': {children: {'td':{children:{'input':{onclick:exportToFile,type:'button',className:'button',value:ratio.texts.exportToFile}}}}}});
				}
				else{
					while(++i<third){
						if(i==0)
			...
			getSelections = function() {
				var rv=[],i=0;
				while(i<ratio.perAll.length)
					rv.push(getSelection(i,ratio.perAll[i++],ratio.perAll[i++]));
				return rv;
			},
			...
			onCellMove = function() {
				...
				if(ratio.perAll[3].def!==0&&lastTipData!==data){
					lastTipData=data;
					ndays=getNrDays()+1;
					...
					if(i<ndays){
						...
						tn=ratio.now-ndays*86400000+3*86400000/4;
						i=ndays;
						if(this.aver){
			...
			renderOcc = function(isPerMs) {
				...
				tds.push({ 'td': { className: 'headingCell', children: {'div': {innerHTML:pushMatrix(isPerMs?ratio.texts.robot:ratio.texts.vc,true)} } } });
				if(!isPerMs)
					tds.push({ 'td': { className: 'headingCell', children: {'div': {innerHTML:pushMatrix(ratio.texts.nrVms)} } } });
				tds.push({ 'td': { className: 'infoCell dayCell', children: {'div': {innerHTML:pushMatrix(ratio.texts.nrDays)} } } });
				cl=9;
				cl=cl<2?100:cl<3?25:cl<4?20:cl<5?15:cl<8?13:6;
				hd=[ratio.texts.totalMilk,
					ratio.texts.totalPercent,	ratio.texts.normalPercent,	ratio.texts.errPercent,	ratio.texts.warnPercent,	ratio.texts.alarmPercent,	ratio.texts.average,
					ratio.texts.occTotal,		ratio.texts.occNormal,		ratio.texts.occErrors,	ratio.texts.occWarnings,	ratio.texts.occAlarms];
//console.log('cl='+cl+' (cl bef='+cll+')');
				...
				jr.foreach(arr, function(o){
					...
					if(tot){
						...
						tds.push({ 'td': { className: 'infoCell'+clName, children: {'div': {innerHTML:noll(sum=getCount(o.totMilk,o.currLastDayIndex))} } } });
						tds.push({ 'td': { className: 'infoCell'+clName, children: {'div': {innerHTML:noll(Math.round((tot/sum)*100))} } } });
						tds.push({ 'td': { val:o.normal, onmouseout:onCellOut, onmousemove:onCellMove, className: 'infoCell'+clName, children: {'div': {innerHTML:noll(tot?Math.round((normal/tot)*100):0)} } } });
						tds.push({ 'td': { val:o.err, onmouseout:onCellOut, onmousemove:onCellMove, className: 'infoCell'+clName, children: {'div': {innerHTML:noll(tot?Math.round((err/tot)*100):0)} } } });
						tds.push({ 'td': { val:o.warn, onmouseout:onCellOut, onmousemove:onCellMove, className: 'infoCell'+clName, children: {'div': {innerHTML:noll(tot?Math.round((warn/tot)*100):0)} } } });
						tds.push({ 'td': { val:o.alarm, onmouseout:onCellOut, onmousemove:onCellMove, className: 'infoCell'+clName, children: {'div': {innerHTML:noll(tot?Math.round((alarm/tot)*100):0)} } } });
						tds.push({ 'td': { aver:1, onmouseout:onCellOut, onmousemove:onCellMove, className: 'infoCell'+clName, children: {'div': {innerHTML:noll(Math.round(aver))} } } });
						tds.push({ 'td': { className: 'infoCell'+clName, children: {'div': {innerHTML:noll(tot)} } } });
						tds.push({ 'td': { className: 'infoCell'+clName, children: {'div': {innerHTML:noll(normal)} } } });
						tds.push({ 'td': { className: 'infoCell'+clName, children: {'div': {innerHTML:noll(err)} } } });
						tds.push({ 'td': { className: 'infoCell'+clName, children: {'div': {innerHTML:noll(warn)} } } });
						tds.push({ 'td': { className: 'infoCell'+clName, children: {'div': {innerHTML:noll(alarm)} } } });
			...
			getNrDays = function() {
				var nrDays=ratio.perAll[3].def;
				nrDays=nrDays==0?0:nrDays==1?6:nrDays==2?20:22;
				return nrDays;
			},
			recalculate = function() {
				renderOcc(ratio.perAll[1].def);
			},
			ms=function(){
...
jr.init( function() {
    ...
    jr.ec( document.body, { children: [
		{ 'div': { style: { left: '0.5%', top: '99%' }, actions: ratio.alarmListener } },
	] } );
	jr.ajax( 'SrvAnimal', 'milkingRatio', {now:ratio.now.getTime(),offs:ratio.now.getTimezoneOffset()}, 'ratioData' );
} );
