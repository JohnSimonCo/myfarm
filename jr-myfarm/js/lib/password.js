...
.factory('appendMobileSuffix', ['$window', function($window) {
	return function(url) {
		var locationIsMobile = $window.location.pathname.indexOf('-mobile.html') !== -1;
		var urlIsMobile = url.indexOf('-mobile.html') !== -1;
		return (locationIsMobile && !urlIsMobile) ? url.replace('.html', '-mobile.html') : url;
	}
}])
.factory('removeMobileSuffix', [function() {
	return function(url) {
		var isMobile = url.indexOf('-mobile.html') !== -1;
		return isMobile ? url.replace('-mobile.html', '.html') : url;
	}
}])
