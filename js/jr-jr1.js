var jr = {
	...
	internal: {
		...
		callId: 0,
		images: {},
		timeStamp: function() {return '?ver=' + (jr.version||window.thisVer); },
	...
	getResource: function(name) {
		return "/Delaval/Resources/" + name + jr.internal.timeStamp(); },
	...
	insertTranslationParams: function( text, params ) {
		...
		if(notUsed.length)
			jr.sendRequest('Logger.log', { level: 'Info', module: 'Xlat', title: "Parameter not used in text: "+notUsed.join(),text: text});
		var missing;
		if(missing = text.match(/\$([0-9a-zA-Z-_])+/g)) {
			jr.sendRequest('Logger.log', { level: 'Alarm', module: 'Xlat', title: "Parameter missing: "+missing.join(),text: text});
		}
		return text;
	},
	...
	AjaxCaller: function () {
		this.invoke = function(controller, method, parameters, eventType, eventParameters, hideError, errorFunc) {
			var ajaxCall = {
				...
				url: '/Delaval/mvc/'+controller+'/'+method+'?i=' + Date.now(),
	...
	ajaxCallback: function( controller, method, parameters, callback, context, errorMethod, dataType ) {
	  return new jr.AjaxCallback().invoke( controller, method, parameters, callback, context, !!errorMethod, errorMethod, dataType );
	},
	...
	callbackForLatest:null,
	broadcastError:function(){
		var bm=jr.broadcastMap;
//		jr.broadcastMap={};
		jr.broadcastId=0;
		jr.broadcastThread!==null&&clearTimeout(jr.broadcastThread);
		jr.broadcastThread=null;
		for(var obj in bm)
			for(var id in bm[obj])
				bm[obj][id].error&&bm[obj][id].error();
		jr.broadcastThread=setTimeout(jr.broadcastTryAgain,5000);
//console.log('Try again to setup broadcast again in 5 sec...');
	},
	broadcastTryAgain:function() {
//console.log('Try again...');
	...
	broadcastLoop:function(){
//console.log('Send poll to server...');
		jr.ajaxCallback('SrvLongPoll','poll',jr.broadcastId,function(data){
			jr.broadcastThread=null;
			if(data){
				...
				while (sd.hasMore()) {
					...
					if (typeof func === 'number') {
						if (jr.callbackForLatest)
							jr.callbackForLatest(vcId, func);
					}
					else {
						var msgLen=sd.getInt(),pos=sd.getPos(),msg=data.substr(pos,msgLen);
						sd.setPos(pos+msgLen);
						if(jr.broadcastMap[func]){
							var cb=jr.broadcastMap[func][vcId];
							if(cb){
								try{
			...
			jr.broadcastThread=setTimeout(jr.broadcastLoop,1000);
		},null,jr.broadcastError);
	},
	onLatest: function(callbackLatest) {
		jr.callbackForLatest = callbackLatest;
	},
	onBroadcast: function(vcId, functionType, callback, errorCallback ) {
		if(!jr.broadcastId)
			jr.broadcastId=Math.floor(Math.random()*0x1000000).toString()+new Date().getTime();
		if(!jr.broadcastMap[functionType])
			jr.broadcastMap[functionType]={};
		jr.broadcastMap[functionType][vcId]={callback:callback,error:errorCallback};
		jr.ajaxCallback('SrvLongPoll','broadcast',jr.broadcastId+','+vcId+','+functionType);
		if(!jr.broadcastThread)
			jr.broadcastThread=setTimeout(jr.broadcastLoop,1000);
	},
