...
(function(name, definition) {
	...
	Router.prototype._buildRequestObject = function(fragmentUrl, params, splat){
		...
		request.query = parseUrlQuery(fragmentUrl.split(/[?](.+)/)[1]);
	...
	function parseUrlQuery(str) {
		...
		var query = {}, pairs = str.split('&'), keyValue, key, value, undef, existing, old;
		...
		for(var i = 0, l = pairs.length; i < l; i++) {
			...
			if(existing) {
				if($.isArray(existing)) {
					addToQueryArray(existing, value, undef);
				} else {
					old = existing;
					query[key] = [old];
					addToQueryArray(query[key], value, undef);
				}
			} else {
	...
	//@ADDED
	function addToQueryArray(array, value, undef) {
		if(!undef) array.push(value);
	}

	//@ADDED
	Router.prototype.urlQuery = function() {
		return parseUrlQuery(location.hash.split(/[?](.+)/)[1]);
	}
	...
	*/
	...
	//@EDITED
	Router.prototype.redirect = function(url, addToHistory){
		if(addToHistory) this.setLocation(url);
		if(!this._paused)
