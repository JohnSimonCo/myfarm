...
.factory('jrGraph', [function() {
	var myGraph = {
		...
		instance:  function(onMosueStopIn, monthsIn) {
			...
			point=function(xFrom, y, xTo, id, col, specialColor){
				this.x = xFrom;
				this.x2 = xTo;
				this.y = y;
				this.id = id;
			...
			plotLine=function(colorLine, thickness, stapleOutlineColor, onMouseOver, markDiameter){
				...
				this.addPoint = function(x, y, x2, id, col, specialColor){
					this.points.push(new point(x, y, x2, id, col, specialColor));
	...
	//					tPos = yPix - fontHeight/2;
				...
				this.paintMarks = function(ctx, ln) {
					if (ln.markDiameter) {
						var i = 0, diam = ln.markDiameter, diam2 = diam << 1;
						ln.markX=[];
						ln.markY=[];
	...
	//								ctx.fillRect(p.xPix - diam, p.yPix - diam, diam2, diam2);
									ctx.beginPath();
									ctx.arc(p.xPix, p.yPix, p.specialColor ? 2 * diam : diam, 0, 2 * Math.PI, false);
				...
				this.addPoint = function(index, x, y, x2, id, col, specialColor) {
					recalculate = true;
					var ln = this.lines[index];
					ln.addPoint(x, y, x2, id, col, specialColor);
					if (firstTime) {
						firstTime = false;
						xMin = ln.xMin;
						yMin = ln.yMin;
						...
						if (ln.markX)
							while (true) {
								...
								if (ii === ln.markX.length)
									break;
								if (ln.markX[ii] <= xr && yu < ln.markY[ii] && yd > ln.markY[ii]) {
			...
			this.addPoint = function(d, value, id, col, specialColor) {
				this.plot.addPoint(currIndex, d, value, null, id, col, specialColor);
			};
