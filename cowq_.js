...
jr.include('/delavalDialog.js');
jr.include('/simpleGraph.js');
jr.include('/milkings.js');
jr.include('/util.js');
jr.include('/modalContainer.css');
function CowQueue(container, id, myScreenPos, ajaxError) {
	var
	texts = {
...
//	openGateAnimals
	...
	onCloseWaitArea=function(){
		jr.ajax('SrvAnimal', 'closeOpenWaitingArea', id, null, null, null, ajaxError);
//console.log("Closing: "+(openGateUntil==null?'null':openGateUntil.toString()));
	...
	doOpenToWaitArea=function() {
		...
		jr.ajax('SrvAnimal', 'setOpenWaitingArea', id+' '+s, null, null, null, ajaxError);
	...
	onSelectCowClick=function(obj, index) {
		if(index<0){
			if(obj.rowData.markBySign==null){
				...
				if(comment!=null)
					jr.ajax('SrvAnimal', 'makeNotification', {targetId:id, animalNr:obj.rowData.nr.toString(), notification:comment}, null, null, null, ajaxError);
			}
			else {
				confirm(texts.confirmDelete+' '+obj.rowData.nr + texts.questionMark, texts.yes, texts.no, function() {
						jr.ajax('SrvAnimal', 'deleteNotification', {targetId:id, animalNr:obj.rowData.nr.toString()}, null, null, null, ajaxError)
					}
				);
			}
		}
...
//					return r;
	...
	cow=function(d){
		...
		this.expectedBuildupDate=getDate(d);
		this.expectedDryOff=getDate(d);
		this.expectedHeatDate=getDate(d);
		this.expectedPregnancyCheck=getDate(d);
		this.lastHeatDate=getDate(d);
		this.lastInseminationDate=getDate(d);
		this.latestSCCDate=getDate(d);
		this.breed=d.getInt();
		this.hoursSinceHighActivity=d.getInt();
		this.latestSCC=d.getInt();
		this.expectedInseminationDue=d.getInt();
		this.expectedInseminationDueDate=getDate(d);
...
//				pg.allCows.splice(i,1);
			...
			showCowDetails=function(){
				...
				if(viewMode===0){
					...
					if(!cowDetail||cowDetail.nr!==cow.nr){
						detailCow=cow;
						jr.ajax('SrvAnimal', 'getAnimalData', id+','+cow.nr, 'cowDetailUpdate', null, null, ajaxError);
					}
				}
			};
		showCowDetails();
	...
	this.initData=function(image,cowQueueData,profileData){
		cowImage=image;
		...
		codeSets={};
		var sd=new JsSerilz('$',cowQueueData.codeSetsSerialized), clen = sd.getInt();
		while (--clen >= 0) {
			var cKey=sd.getString();
			codeSets[cKey]=sd.getString();
		}
...
//	$( window ).resize( onresize );
	jr.eventManager.addListener('cowqUpdate', this, function(data) { onNewData(data);});
	jr.eventManager.addListener('cowDetailUpdate', this, function(data) { onCowDetailUpdate(data);});
